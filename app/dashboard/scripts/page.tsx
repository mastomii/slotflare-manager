...
export default function ScriptsPage() {
  ...
  const generateScriptPreview = (script: ScriptData) => {
    return `// Attach an event listener to handle 'fetch' events
...
async function handleRequest(request) {
    ...
    const whitelistPaths = [${(script.whitelistPaths || []).map(p => `'${p}'`).join(', ')}];

    // List of forbidden keywords to block if found in the content
    const forbiddenKeywords = [${(script.keywords || []).map(k => `'${k}'`).join(', ')}];
    ...
    if (containsForbidden) {
        ...
        if (${script.enableAlert}) {
            try {
                ...
                fetch('${typeof window !== 'undefined' ? window.location.origin : 'https://slotflare-manager.vercel.app'}/api/trigger', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
